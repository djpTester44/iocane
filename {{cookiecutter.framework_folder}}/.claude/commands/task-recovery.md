...
- **SEAM_ENTRY_MISSING:** Use `seam_parser.find_by_component(seams, name)` to read the component's seam entry from `plans/seams.yaml`, then project via the standalone function `seam_parser.to_seam_entry(comp)` and embed it in `seam_context` (fields: `injected_contracts`, `key_failure_modes`, `external_terminal` only).
