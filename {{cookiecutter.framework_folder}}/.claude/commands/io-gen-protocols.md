...
2. `plans/symbols.yaml` exists and parses. Every exception name appearing in any `MethodSpec.raises` is declared as `kind: exception_class` with `declared_in: src/...`. Every custom type appearing in any `ArgSpec.type_expr` or `MethodSpec.return_type` that is not a Python builtin is declared as `kind: shared_type` with `declared_in: src/...`.
