...
**Objective:** Execute one checkpoint's task file exactly as specified. Implement the component against pre-existing contract tests + connectivity tests, authoring emergent impl tests via TDD as internal logic develops. Write a status file on completion. Terminate.
...
- **Protocol contract:** [contract path named in task file]
...
- **Goal:** Understand the contract surface you are required to implement.
- **Rule:** The CDT and CT tests shape the design of the impl. Build minimal code to pass the CDT, CT, and any emergent impl tests — do not add public methods the tests don't call. All three layers must pass for success.
...
component contract, per CDD (`.claude/rules/cdd.md`).
...
- **Rules:**
  ...
  - If a TDD cycle reveals the contract should differ,
    HALT with a FAIL status naming the contract gap -- do NOT silently
...
- **Rules:**
  ...
  - Follow the contract surface exactly — method names, parameter types, return types
...
- **Rule:** All CT gates must pass. Any failure is an escalation
  trigger. Do not attempt autonomous remediation (CT body
  re-authoring is forbidden). Escalate
...
| Component contract silent on a necessary precondition, return semantic, or collaborator | Write FAIL status naming the contract gap, terminate |
