...
* **Action:** Populate `catalog.toml` at the repo root from the input sources below. Use typed entry names (lowercase, identifier-safe) that match the project's domain language.
  ...
  | `external_systems` | `plans/PRD.md` (Tech Stack, Integrations, External Dependencies sections) + Step B.5 trust-edge analysis (in-session) + `plans/roadmap.md` Trust Edges section (just written in Step E) | Third-party services or APIs the system calls -- payment, auth, analytics, search, AI/ML provider, etc. Infer `kind` by provider class or service category. Set `trust_boundary` per the Rule below (mirrors the roadmap's TE-NN entries written in this same run). |
...
* **Rule for `external_systems.trust_boundary`:** The flag is a forward-looking structured-index mirror of the roadmap's Trust Edges section. The roadmap (`plans/roadmap.md`, just written in Step E) remains authoritative; the flag is a structured-index optimization for the Phase 1 roadmap-tier trust-edge gate (`validate_trust_edge_chain.py`) and is NOT itself the source of truth for adversarial-edge declarations. For each inferred `external_systems` entry, set `trust_boundary = true` when the entry's name resolves to the owning component (or backing service) of a `TE-NN` entry written in this same `/io-specify` run -- the agent has the context to make this judgment because Step B.5 produced the TEs and Step E just wrote them. Otherwise set `trust_boundary = false`. Per `decisions.md` D-03 + `00-framework-adoption.md` §4.5 row 4, Phase 3 implementation MUST NOT compete with the Phase 1 roadmap-tier gate; the flag never substitutes for the roadmap declaration. Operator review of the populated catalog (operator-owned, post-write, before `/io-architect`) focuses on the FALSE set -- entries that should mirror a roadmap TE but don't (typically because a TE was missed at Step B.5). Entries already set TRUE trace directly to a TE-NN authored in this run and need no re-asking.
