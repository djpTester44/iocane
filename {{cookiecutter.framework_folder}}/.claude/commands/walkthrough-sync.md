...
>
> 1. Load the walkthrough template: `view_file .claude/templates/walkthrough.md`
> 2. Run the context generator:
>
> ```bash
> // turbo
> uv run python .claude/scripts/generate_walkthrough_context.py
> ```
>
...
Follow the template structure in `.claude/templates/walkthrough.md`. For each section:
...
| `.claude/templates/walkthrough.md` | Read-only during generation |
