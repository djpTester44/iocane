...
**Preservation rule:** Treat `plans/component-contracts.yaml` as the authoritative component list. If a seam entry corresponds to a component still registered there, do **not** auto-remove that seam entry just because the implementation file does not exist yet. CRC data source is `component-contracts.yaml`; the project-spec.md CRC section is rendered output (generated by `render_crc.py`).
