...
- **Feature fan-out cap:** max 2 roadmap features per CRC. A component serving 3+ features from `plans/roadmap.md` must be split along feature boundaries. **Every component that carries feature logic MUST declare the feature IDs** (e.g., `F-01`, `F-02`) it supports -- they are written to the `features:` field in Step F-3 and are what the budget validator reads. An empty `features:` is reserved for shared infrastructure that legitimately has no direct feature fan-out (e.g., `Settings`, loggers); the validator emits a non-blocking warning if a behavioral component (has declared responsibilities) leaves `features:` empty, so A.1b cannot be silently bypassed by forgetting to declare. **Shared-infra primitives** -- loggers, factories, clients (HTTP, database, queue), retry policies, secret clients -- MUST author with `features: []` from Step F authoring time. Tagging them with feature IDs causes `validate_trust_edge_chain.py` Check 2 multi-owner resolution to demand adversarial raises from the primitive -- but the trust boundary is owned by the behavioral component that uses the primitive, not by the primitive itself. Authoring `features: []` upfront eliminates the mid-Step-G de-tagging cycle.
- **Composition-root decomposition:** a `composition_root: true` component with 3+ Layer-2/3 collaborators (domain + infrastructure, excluding other composition roots) must decompose into resource-scoped sub-components -- one router/handler/sub-app per resource, each wiring at most 2 Layer-2/3 collaborators. **Two valid remediation paths under A.1c.** When cap pressure comes from domain or application-tier collaborators (resource handlers, business services), the path is decomposition: split along resource boundaries. When cap pressure comes from cross-cutting infrastructure (logging, tracing, metrics, request-ID middleware), the path is removal from the composition root's collaborator list: cross-cutting infrastructure is conventionally wired at the process startup script (uvicorn entry, celery worker entry, scheduler entry) before the composition root is constructed, not inside the composition root's collaborator graph. Choose the path that matches the source of the cap pressure; the validator stays unchanged as the deterministic backstop.
...
The default invocation (no flags) is correct for any project that already has a `pyproject.toml` -- including projects whose `pyproject.toml` lacks a `[tool.importlinter]` section, which is handled via strip-zero-or-more + always-append semantics. Pass `--bootstrap` ONLY in the explicit no-pyproject case (the project has no `pyproject.toml` at all); it copies from `.claude/templates/pyproject.toml` first. Do not reflexively pass `--bootstrap` -- it is a brownfield-without-pyproject affordance, not a default.
...
- `validate_symbols_coverage.py` exits 1 on uncovered project-custom exceptions OR symbol conflicts (existing class), and exits 2 on `used_by` referential integrity OR reciprocity drift (new class). Tie-break: 1 wins if both classes fail in the same run. Stdlib and builtin exceptions are correctly skipped.
...
- **FINDINGS:** evaluator emits OBSERVATION findings to a path printed on stdout. **The architect MUST read each emitted finding YAML in full** (`diagnosis.what`, `diagnosis.why`, `remediation.fix_steps` fields) before composing the Step I output. Surface all findings paths at Step I alongside the canonical artifacts summary. Do NOT revise the artifact set or re-run Step F in response to evaluator findings within this invocation -- Step I is the operator gate. The operator decides the next action (approve / in-place corrections / upstream revision).
...
Step H semantic critic: PASS / FINDINGS
  design-evaluator: PASS|[N] findings emitted at .iocane/findings/<paths>
    - <path basename>: <defect_kind> on <where> -- <one-line gist of diagnosis.what>
    - <path basename>: <defect_kind> on <where> -- <one-line gist of diagnosis.what>
