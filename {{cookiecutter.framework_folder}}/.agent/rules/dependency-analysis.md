...
2.  **During `/io-loop` verify steps**: Run `uv run lint-imports`.
3.  **During `/review` pre-scan**: Run `uv run lint-imports` to surface coupling and contract compliance.
...
### Visual Exploration
```bash
uv run import-linter explore <package>
```
e.g. `uv run import-linter explore lib` OR `uv run import-linter explore jobs`
