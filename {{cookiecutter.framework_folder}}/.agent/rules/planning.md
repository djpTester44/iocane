...
3.  **Context Locking**: Every task in `tasks.json` **MUST** include a `context_files` array listing the exact `.py` and `.pyi` files required to complete that specific step. Do not list the entire repository.
    - **[HARD] Rejection Gate:** If any task has an empty `context_files` array, the tasking output is REJECTED. Vague tasks cause grep-spam and lazy-dump violations downstream. Treat missing `context_files` as a blocking defect.
...
3.  **Integration Integrity**: If a Checkpoint modifies the critical path (Input -> Logic -> Output), you **MUST** include a task to run/update `tests/test_integration_golden_path.py`.
